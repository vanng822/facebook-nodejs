...
const USER_AGENT = 'facebook-nodejs/' + version + ' (+https://github.com/vanng822/facebook-nodejs)';
/**
 * Prefix to avoid name collision in session and cookies
 */
const FB_SESSION_KEY_PREFIX = 'fbsk_';
const FB_SR_COOKIE_PREFIX = 'fbsr_';

const DEFAULT_USER_FIELDS = 'uid, name';

/**
 * For different request www is for login dialog
 *
 */
const DOMAIN_MAP = {
...
Object.freeze(DOMAIN_MAP);

const DROP_QUERY_PARAMS = ['code', 'state', 'signed_request'];
Object.freeze(DROP_QUERY_PARAMS);
/* allowing persistence */
const SUPPORT_KEYS = ['state', 'code', 'access_token', 'user_id', 'expires'];
Object.freeze(SUPPORT_KEYS);
...
var apiVersion = '';
/*
 * data only store when user a redirect for login and no clean up until next
 * login
 */
/* application which uses this data should have that responsibility */
/**
 * Data: { body: req.body, params: req.params, query: req.query, files:
 * req.files }
 */
...
function authenticate(req, res, next) {
	var code, state, signedRequest, accessToken, expires, i, len;
...
var saveRequestData = function(req) {
    ...
    req.session[REQUEST_DATA_SESSION_KEY] = {
        body: req.body,
        params: req.params,
        query: req.query,
        files: req.files
 ...
 *                res
 * @param {Function}
 *                clearCookie clearCookie is optional And will be called with
 *                the name of signed request cookie. Default behavior is to try
 *                using setHeader from ServerResponse class Can be good if
 *                application has a better cookie handler
 */
...
Facebook.prototype = {
	...
	_build_options : function(path, method, params) {
		var queryParams = '';
		if (params) {
			queryParams += this._params_to_query(params) + '&';
		}

		if (this._getAccessToken()) {
			queryParams += 'access_token=' + this._getAccessToken();
		}
		if (queryParams) {
			if(path.indexOf('?') !== -1) {
...
/**
 * Connected to Object me for doing query like /me/connectionType, for instance
 * /me/friends
 */
		...
		r.on('end', function() {
			...
			if(r.statusCode === 200) {
				try {
					...
					/*
					 * expires is relative so adding from
					 * now
					 */
