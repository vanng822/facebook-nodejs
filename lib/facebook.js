...
var redirectLoginForm = module.exports.redirectLoginForm = function(req, res) {
	...
	query = '/dialog/oauth?client_id=' + appId + '&redirect_uri=' + currentUrl + '&scope=' + scope + '&state=' + state;
	
	redirect(res, PROTOCOL + DOMAIN_MAP.www + query);
};
			...
			res.on('end', function() {
				try {
					if(res.statusCode == 200) {
						...
						callback(new FacebookError(data), null);
					}
		...
		r.on('end', function() {
			...
			if(r.statusCode == 200) {
				try {
					...
					callback(null, parsedUrl.query.access_token, Math.floor(Date.now() / 1000) + parsedUrl.query.expires);
				} catch(e) {
					console.error(e);
				...
				console.error(data);
				callback(new FacebookError(data), null, null);
			}
		});
	});
}
